...
num_sources = 1
seg_angles = 64
file_header = f"A20221016_{a_num}x{a_num}_{num_sources}src_{seg_angles}_v1.1"
...
# own_colors[:, 0] = np.linspace(255/256, 255/256, N) # R = 255
# own_colors[:, 1] = np.linspace(248/256, 193/256, N) # G = 232
# own_colors[:, 2] = np.linspace(220/256, 37/256, N)  # B = 11
# own_colors[:, 0] = np.linspace(255/256, 238/256, N) # R = 255
# own_colors[:, 1] = np.linspace(248/256, 173/256, N) # G = 232
# own_colors[:, 2] = np.linspace(220/256, 14/256, N)  # B = 11
# own_colors[:, 0] = np.linspace(255/256, 255/256, N) # R = 255
# own_colors[:, 1] = np.linspace(246/256, 153/256, N) # G = 232
# own_colors[:, 2] = np.linspace(143/256, 18/256, N)  # B = 11
# own_colors[:, 0] = np.linspace(255/256, 205/256, N) # R = 255
# own_colors[:, 1] = np.linspace(246/256, 173/256, N) # G = 232
# own_colors[:, 2] = np.linspace(143/256, 0/256, N)  # B = 11
...
def main(seg_angles, model, recordpath, sim_parameters):
    ...
    while SIM_TIME >= time:
        ...
        if step%SIM_STEP==0:
            ...
            ax1.set_xlabel('x')
            ax1.set_ylabel('y')
            #ax1.grid(color='k', linestyle='-', linewidth=12)
