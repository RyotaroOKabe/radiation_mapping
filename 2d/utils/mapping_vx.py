...
fig_header = f'A20220912_{a_num}x{a_num}_{num_sources}src_{seg_angles}_v1.1.1'
...
def main():
    ...
    for filename in files:
        ...
        if save_process:
            ...
            plt.show()
            savedict = {'m': m, 'RSID': RSID, 'hxTrue_data': hxTrue_data, 'pose_x': pose_x, 'pose_y': pose_y, 'filename': filename}
            with open(f'{datapath}/{filename[:7]}.pkl', 'w') as f:
                pkl.dump(savedict, f)
