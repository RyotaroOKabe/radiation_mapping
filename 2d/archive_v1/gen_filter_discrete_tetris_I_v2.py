...
def gen_materials_geometry_tallies(panel_density, e_filter, *energy):
    ...
    #for S3 layer
    min_x3 = openmc.XPlane(x0=-2, boundary_type='transmission')     #!20220715 size-change!
    max_x3 = openmc.XPlane(x0=+2, boundary_type='transmission')
    min_y3 = openmc.YPlane(y0=-0.5, boundary_type='transmission')
    max_y3 = openmc.YPlane(y0=+0.5, boundary_type='transmission')
    ...
    assembly.lower_left = [-2, -0.5]  #[-5, -1.5]  #[-0.5, -2]    #!20220626  # size-change! (x, y)
    #assembly.universes = [[cell_universe] * 2] * 2  #! size-change!
    assembly.universes = [[cell_universe] * 4] * 1 #[[cell_universe] * 10] * 3  #! size-change! ([[cell_universe] * num_x] * num_y ) (x, y)
    ...
    plt.show()   #!20220117
    plt.savefig('save_fig/geometry_tetris_largeI.png')   #!20220117
    plt.close()
    ...
    mesh.dimension = [4, 1] #[10, 3] #[1, 4] #[2, 2]   #! size-change! (x, y)
    mesh.lower_left = [-2, -0.5] #[-5, -1.5] #[-0.5, -2]#[-1, -1]  #[-10, -10]     #!20220715   #! size-change! (x, y)
...
def process_aft_openmc(folder1='random_savearray/', file1='detector_1source_20220118.txt', \
    ...
    mean = fiss['mean'].values.reshape((1, 4)) #reshape((3, 10))  #reshape((1, 4))  #reshape((2, 2)) # numpy array   #!20220715   #! size-change! (y, x)
    ...
    #print(absorb.shape)
    #print("std_dev")
    stdev = absorb.std_dev.reshape((1, 4)) #reshape((3, 10))  #reshape((2, 2))     #! size-change!
    stdev_max = stdev.max()
    #print(stdev)
    ...
    mean_list=mean.T.reshape((1, 4)).tolist() #reshape((1, 30)).tolist()    #! size-change! (x, y)
...
if __name__ == '__main__':
    ...
    folder1='openmc/disc_filter_tetris_I_data_20220715_v1.2/'
    folder2='openmc/disc_filter_tetris_I_fig_2022715_v1.2/'
