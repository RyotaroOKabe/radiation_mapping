...
model_path = 'save_model/model_20220627_openmc_3cm_3x3_10000_ep500_bs256_dsc_new_test_v1.1_model.pt'
...
def main():
    ...
    while SIM_TIME >= time:
        ...
        if step%SIM_STEP==0:
            ...
            #=========================#!20220509
            xdata_original=det_output.reshape(3, 3)  #!size-change
...
def gen_materials_geometry_tallies(panel_density, e_filter, *energy):
    ...
    #for S3 layer
    min_x3 = openmc.XPlane(x0=-1.5, boundary_type='transmission')     #!20220124 #!size-change
    ...
    assembly.lower_left = [-1 * 3 / 2.0] * 2    #!20220124 #!size-change
    assembly.universes = [[cell_universe] * 3] * 3 #!size-change
    ...
    mesh.dimension = [3, 3]  #!size-change
    mesh.lower_left = [-1.5, -1.5]  #[-10, -10]     #!20220124  #!size-change
...
def process_aft_openmc_v1(folder1='random_savearray/', file1='detector_1source_20220118.txt', \
    ...
    mean = fiss['mean'].values.reshape((3, 3)) # numpy array   #!20220118    #!size-change
    ...
    #print(absorb.shape)
    #print("std_dev")
    stdev = absorb.std_dev.reshape((3, 3))    #!size-change
    stdev_max = stdev.max()
    #print(stdev)
    ...
    mean_list=mean.T.reshape((1, 9)).tolist()  #!size-change
