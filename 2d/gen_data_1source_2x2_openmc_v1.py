...
model_path = 'save_model/model_20220627_openmc_2cm_2x2_10000_ep500_bs256_dsc_new_test_v1.1_model.pt'
...
def main():
    ...
    while SIM_TIME >= time:
        ...
        if step%SIM_STEP==0:
            ...
            #=========================#!20220509
            xdata_original=det_output.reshape(2, 2)  #!size-change
...
def gen_materials_geometry_tallies(panel_density, e_filter, *energy):
    ...
    assembly.lower_left = [-1 * 2 / 2.0] * 2    #!20220124 #!size-change
    ...
    mesh.dimension = [2, 2]  #!size-change
    mesh.lower_left = [-1, -1]  #[-10, -10]     #!20220124  #!size-change
...
def process_aft_openmc_v1(folder1='random_savearray/', file1='detector_1source_20220118.txt', \
    ...
    mean = fiss['mean'].values.reshape((2, 2)) # numpy array   #!20220118    #!size-change
    ...
    #print(absorb.shape)
    #print("std_dev")
    stdev = absorb.std_dev.reshape((2, 2))    #!size-change
    stdev_max = stdev.max()
    #print(stdev)
