...
def gen_materials_geometry_tallies(panel_density, e_filter, *energy):
    ...
    #for S3 layer
    min_x3 = openmc.XPlane(x0=-1, boundary_type='transmission')     #!20220124
    max_x3 = openmc.XPlane(x0=+1, boundary_type='transmission')
    min_y3 = openmc.YPlane(y0=-1, boundary_type='transmission')
    max_y3 = openmc.YPlane(y0=+1, boundary_type='transmission')
    ...
    assembly.lower_left = [-1 * 2 / 2.0] * 2    #!20220124 # size!
    assembly.universes = [[cell_universe] * 2] * 2
    ...
    mesh.dimension = [2, 2] #[10, 10]   #!cell size!
    mesh.lower_left = [-1, -1] #[-5, -5]  #[-10, -10]    #!cell size!
...
def process_aft_openmc_v1(folder1='random_savearray/', file1='detector_1source_20220118.txt', \
    ...
    mean = fiss['mean'].values.reshape((2, 2)) # numpy array   #!20220626 cell size!
    ...
    stdev = absorb.std_dev.reshape((2, 2))  #!cell size!
    stdev_max = stdev.max()
    #print(stdev)
    ...
    mean_list=mean.T.reshape((1, 4)).tolist() #!cell size!
...
def after_openmc(rad_dist, rad_angle, folder1, folder2):      
...
if __name__ == '__main__':
    ...
    dist_min = 100
    dist_max = 100
    folder1='openmc/discrete_2x2_data_20220627_v1/'
    folder2='openmc/discrete_2x2_fig_20220627_v1/'
    ...
    for i in range(num_data):
        ...
        before_openmc(dist, rad_angle, num_particles)
        openmc.run()
        mm = after_openmc(dist, rad_angle, folder1, folder2)
