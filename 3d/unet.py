...
class DoubleConv_3D(nn.Module):    #!20220701
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.double_conv = nn.Sequential(
            #nn.Conv1d(in_channels, mid_channels, kernel_size=3, padding=2, padding_mode='circular'),    #!20211230      #?tentative
            nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding=1, padding_mode='circular'),    #!20220701      #?tentative
            # nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),    #!20211230
            #nn.Conv1d(mid_channels, out_channels, kernel_size=3, padding=2,padding_mode='circular'),    #!20211230      #?tentative
            nn.Conv2d(mid_channels, out_channels, kernel_size=3, padding=1, padding_mode='circular'),    #!20220701     #?tentative
            # nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)    #!20211230
        )
...
class Up_3D(nn.Module):    #!20220701
    """Upscaling then double conv"""
    ...
    def __init__(self, in_channels, out_channels):
        ...
        #self.up = nn.ConvTranspose1d(in_channels , in_channels // 2, kernel_size=2, stride=2)  #!20220102
        #self.up = nn.ConvTranspose2d(in_channels , in_channels//2, kernel_size=3, stride=2, padding=1, output_padding=1)  #!20220104
        self.up = nn.ConvTranspose2d(in_channels , in_channels//2, kernel_size=3, stride=(1,2), padding=1, output_padding=(0,1)) #  #!20220703
...
class UNet_3D(nn.Module):  #!20220701
    def __init__(self,in_channels=2, c1=16):
        ...
        #self.max_pool = nn.MaxPool1d(2)
        self.max_pool = nn.MaxPool2d((1,2), stride=(1,2)) #nn.MaxPool2d(2)  #!20220703
