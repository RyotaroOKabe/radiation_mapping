...
def process_aft_openmc_v1(folder1='random_savearray/', file1='detector_1source_20220118.txt', \
    ...
    for xa in range(5):
        ...
        ax.imshow(mean[xa, :, :], interpolation='nearest', cmap="plasma")       #!20220118
        ax.set_xlabel('z')  #!20220502 Adjust the incorrect axis setting!
        ax.set_ylabel('y')  #!20220502 Adjust the incorrect axis setting!
        #plt.colorbar()
        ax.set_title("X=" + str(xa))
    for ya in range(5):
        ax = axs[1, ya]
        #axes.append(fig.add_subplot(3, 5, ya+6) )
        ax.imshow(mean[:, ya, :], interpolation='nearest', cmap="plasma")       #!20220118
        ax.set_xlabel('z')  #!20220502 Adjust the incorrect axis setting!
        ax.set_ylabel('x')  #!20220502 Adjust the incorrect axis setting!
        #plt.colorbar()
        ax.set_title("Y=" + str(ya))
    for za in range(5):
        ax = axs[2, za]
        #axes.append(fig.add_subplot(3, 5, za+11) )
        ax.imshow(mean[:, :, za], interpolation='nearest', cmap="plasma")       #!20220118
        ax.set_xlabel('y')  #!20220502 Adjust the incorrect axis setting!
        ax.set_ylabel('x')  #!20220502 Adjust the incorrect axis setting!
        #plt.colorbar()
        ax.set_title("Z=" + str(za))
