...
from dataset_a3_v1 import *  #!20220508
...
class Filterlayer2(nn.Module):
    ...
    def __init__(self, out_features, filterdata):
        ...
        #temp = torch.torch.from_numpy(filterdata.data.reshape((2,40,-1))) #!20211230   #!20220701 out
        temp = torch.torch.from_numpy(filterdata.data.reshape((2,18,40,-1))) #!2020701 (2,18,40,-1)? (2,40,18,-1)? (2,40,-1) ? (18,40,-1)? (2,18,40,-1) ?
        #print(temp) #!20220305
        ...
        #self.weight1 = torch.nn.Parameter(data=torch.t(temp[0,:,:]), requires_grad=True)#self.weight[:,:40] #!20211230 #!20220701 out
        #self.weight2 = torch.nn.Parameter(data=torch.t(temp[1,:,:]), requires_grad=True)#self.weight[:,40:] #!20211230 #!20220701 out
        #?self.weight1 = torch.nn.Parameter(data=torch.t(temp[0,:,:,:]), requires_grad=True)#self.weight[:,:40] #(18, 40, 125) #!20220701
        #?self.weight2 = torch.nn.Parameter(data=torch.t(temp[1,:,:,:]), requires_grad=True)#self.weight[:,40:] #(18, 40, 125) #!20211230
        ...
        self.bias1 = torch.nn.Parameter(data=torch.zeros(1,18,40), requires_grad=True)   # (1, 18, 40)  #!20220701
        self.bias2 = torch.nn.Parameter(data=torch.zeros(1,18,40), requires_grad=True)
    ...
    def forward(self,x):    # x: (N, 2, 18, 40)? or  (N, 1, 18, 40)? or  (N, 18, 40)? or (N, a^3)? or (N)
        ...
        #print(out)  #!20220306
        out = out.view(out.shape[0], 2, 18, 40)    # (N, 36, 40) >> (N, 2, 18, 40)
        #print("========out=======")
        #print(out)  #!20220306
...
class MyNet2(nn.Module):
    def __init__(self, out_features, filterdata):   #!20220701
        ...
        self.l1 = Filterlayer2(out_features, filterdata)#.to(device=DEFAULT_DEVICE, dtype=DEFAULT_DTYPE)    #!20220701
        #self.filterbias = torch.zeros(1,80, requires_grad=True) -0.5
        #self.unet = UNet(c1 = 32)  #!20220701 out
        self.unet = UNet_3D(c1 = 32)   #!20220701
        # print 'haha'
    ...
    def train(self, optim, train, val, epochs,batch_size, acc_func=None, check_overfit_func=None,verbose=10, save_name='output_record'):
        ...
        for i in range(epochs):
            ...
            for j in range(times):
                ...
                loss = loss_train(data_y.transpose(1,2), output.transpose(1,2))   #! Debug this part!!
...
from emd_ring_torch_3d import emd_loss_ring_3d
...
if __name__ == '__main__':
    ...
    #=========================================================
    save_name = "openmc_5cmxx3_ep1000_bs256_20220704_v3.2"      #!20220126
    #=========================================================
    path = 'openmc/discrete_data_20220701_3^3_v1'    #!20220630
    ...
    train_set,test_set=load_data(test_size=50,train_size=None,test_size_gen=None,output_fun=get_output,path=path,source_num=[1],prob=[1.],seed=None) #load_data(50, source_num=[1]) 
    ...
    model.train(optim,train_set,test_set,epochs=1000,batch_size=256,acc_func=None, verbose=10, save_name=save_name)    #!20220126
