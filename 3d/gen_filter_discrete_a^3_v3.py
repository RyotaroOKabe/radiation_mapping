...
def get_output(source):
    sec_center=np.linspace(-np.pi,np.pi,41)
    sec_th=np.linspace(np.pi/40,np.pi*39/40,20) # sec_th=np.linspace(np.pi/36,np.pi*35/36,18)
    output_ph=np.zeros(40)
    output_th=np.zeros(20)  #output_th=np.zeros(18)
    sec_dis_ph=2*np.pi/40.
    sec_dis_th=np.pi/20.    #sec_dis_th=np.pi/18.
...
if __name__ == '__main__':
    ...
    #?dist_min = 100
    #?dist_max = 100
    folder1=f'openmc/disc_filter_data_20220705_{a_num}^3_v1/'
    folder2=f'openmc/disc_filter_fig_20220705_{a_num}^3_v1/'
    #header = "data"
    ...
    theta_list = [90/theta_num+b*180/theta_num for b in range(theta_num)]
