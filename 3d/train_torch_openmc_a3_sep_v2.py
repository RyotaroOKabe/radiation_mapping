...
#         super(MyNet2, self).__init__()
#         # self.l1 = Filterlayer2(ph_num, th_num, filterdata)#.to(device=DEFAULT_DEVICE, dtype=DEFAULT_DTYPE)    #!20220701
#         self.l1_ph = Filterlayer2_ph(ph_num, th_num, filterdata)#.to(device=DEFAULT_DEVICE, dtype=DEFAULT_DTYPE)    #!20220714
#         self.l1_th = Filterlayer2_th(ph_num, th_num, filterdata)#.to(device=DEFAULT_DEVICE, dtype=DEFAULT_DTYPE)    #!20220714
#         self.unet_ph = UNet_ph(in_channels=2, c1 = 32)   #!20220711
#         self.unet_th = UNet_th(in_channels=2, c1 = 32)   #!20220711

#     def forward(self, x):
#         #ph, th = self.l1(x)
#         ph = self.l1_ph(x) #!20220714
#         th = self.l1_th(x)
#         #print(ph.shape)
...
class MyNet2_ph(nn.Module): #!20220714
    def __init__(self, UNet_ph, ph_num, th_num, filterdata):   #!20220701
...
class MyNet2_th(nn.Module):    #!20220714
    def __init__(self, UNet_th, ph_num, th_num, filterdata):   #!20220701
...
class Model(object):
    ...
    def __init__(self, net_ph, net_th, loss_train_ph, loss_train_th, loss_val_ph, loss_val_th, reg=0.):  #!20220711
        super(Model, self).__init__()
        
        #self.net = net
        self.net_ph = net_ph    #!20220714
        self.net_th = net_th
    ...
    def train(self, optim_ph, optim_th, train, val, epochs,batch_size, acc_func_ph=None, acc_func_th=None, check_overfit_func=None,verbose=10, save_name='output_record'):   #!20220714
        #net = self.net
        net_ph = self.net_ph  #!20220714
        net_th = self.net_th
        ...
        for i in range(epochs):
            ...
            for j in range(times):
                ...
                #optim.zero_grad()
                optim_ph.zero_grad()   #!20220714
                optim_th.zero_grad()

                
                #output = net(data_x)    #!20220711 out
                #output_ph, output_th = net(data_x)    #!20220711
                output_ph = net_ph(data_x)    #!20220711
                output_th = net_th(data_x)    #!20220711
                ...
                # for param in net.parameters():
                #     pass
                for param in net_ph.parameters():   #!20220714
                    pass
                for param in net_th.parameters():
                    pass
            ...
            with torch.no_grad():
                for data in datas:
                    ...
                    output_ph = net_ph(data[0])   #!20220714
                    output_th = net_th(data[0])   #!20220714
    ...
    def plot_test(self,test,indx, save_name):
        ...
        with torch.no_grad():
            ...
            predict_test_ph = self.net_ph(torch.as_tensor(test_x))   #!20220714
            predict_test_th = self.net_th(torch.as_tensor(test_x))   #!20220714
            predict_test_ph = predict_test_ph.cpu().detach().numpy()  #!20220714
            predict_test_th = predict_test_th.cpu().detach().numpy()
            

        #fig = plt.figure(figsize=(50, 14))  #!20220701
        fig = plt.figure(figsize=(12, 6), facecolor='white')  #!20220707
        ...
        ax11.plot(phi_list, np.squeeze(test_y_ph),label='Simulated')  #!20220714
        #ax11.set_title(f"[Simulated]\nPeak: \u03C6 = {phi_list[id_ref_ph]} deg ({id_ref_ph})")
        #ax12.plot(test_y_th, theta_list)
        ax12.plot(np.squeeze(test_y_th), theta_list,label='Simulated')    #!20220714
        ...
        #ax11.plot(phi_list, predict_test_ph)
        ax11.plot(phi_list, np.squeeze(predict_test_ph),label='Predicted')    #!20220714
        ax11.set_title(f"[Simulated] \u03C6 = {phi_list[id_ref_ph]} deg ({id_ref_ph})\n[Predicted] \u03C6 = {phi_list[id_pred_ph]} deg ({id_pred_ph})")
        #ax12.plot(predict_test_th, theta_list)
        ax12.plot(np.squeeze(predict_test_th), theta_list,label='Predicted')  #!20220714
...
if __name__ == '__main__':
    ...
    #=========================================================
    save_name = "openmc_3cmxx3_ep1000_bs256_20220715_v1.3"      #!20220126
    #=========================================================
    ...
    net_ph = MyNet2_ph(UNet_ph, ph_num=ph_num, th_num=th_num, filterdata=filter_data2)   #!20220714
    net_ph = net_ph.to(device=DEFAULT_DEVICE, dtype=DEFAULT_DTYPE)

    net_th = MyNet2_th(UNet_th, ph_num=ph_num, th_num=th_num, filterdata=filter_data2)   #!20220714
    ...
    loss_train_ph = nn.MSELoss() #lambda  y, y_pred: emd_loss_ring(y, y_pred, r=2)    #!20220711
    ...
    loss_val_ph = lambda y, y_pred: emd_loss_ring(y, y_pred, r=1).item()    #!20220714
    ...
    model = Model(net_ph, net_th, loss_train_ph, loss_train_th, loss_val_ph, loss_val_th,reg=0.001)  #!20220714
    ...
    # base_params = filter(lambda p: id(p) not in l1_params,
                         # net.parameters())
    optim_ph = torch.optim.Adam([  #!20220714
        #{"params": net.unet.parameters()},
        {"params": net_ph.unet_ph.parameters()},  #!20220711
    ...
    optim_th = torch.optim.Adam([  #!20220714
        #{"params": net.unet.parameters()},
    ...
    model.train(optim_ph,optim_th,train_set,test_set,epochs=1000,batch_size=256, acc_func_ph=None, acc_func_th=None, verbose=10, save_name=save_name)   #!20220711
