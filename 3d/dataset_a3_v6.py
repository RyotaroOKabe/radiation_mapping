...
def plot_angles_separately2(data_index, path, filterpath, filter_types_dict, a_num): #!20220701
    ...
    id_t_m, id_p_m = [int(np.where(dm==dm.max())[i]) for i in range(2)]
    dm_ph = np.sum(relu_np(dm), axis=0)  #!!
    id_sum_ph = int(np.where(dm_ph==dm_ph.max())[0])
    dm_th = np.sum(relu_np(dm), axis=1)
    id_sum_th = int(np.where(dm_th==dm_th.max())[0])

    #--
    dm_ph = (dm_ph -  dm_ph.mean())/dm_ph.std()
    dm_th = (dm_th -  dm_th.mean())/dm_th.std()
    #--
    ...
    # do0_mean = do0.mean()
    # do0_std = do0.std()
    do0 = (do0 -  do0.mean())/do0.std()
    id_t_0, id_p_0 = [int(np.where(do0==do0.max())[i]) for i in range(2)]
    do_ph0 = np.sum(relu_np(do0), axis=0)  #!!
    id_sum_ph0 = int(np.where(do_ph0==do_ph0.max())[0])
    do_th0 = np.sum(relu_np(do0), axis=1)
    id_sum_th0 = int(np.where(do_th0==do_th0.max())[0])

    #--
    do_ph0 = (do_ph0 -  do_ph0.mean())/do_ph0.std()
    do_th0 = (do_th0 -  do_th0.mean())/do_th0.std()
    #--
    ...
    #--
    do_ph1 = (do_ph1 -  do_ph1.mean())/do_ph1.std()
    do_th1 = (do_th1 -  do_th1.mean())/do_th1.std()
    #--
    ...
    # do2_mean = do2.mean()
    # do2_std = do2.std()
    do2 = (do2 -  do2.mean())/do2.std()
    id_t_2, id_p_2 = [int(np.where(do2==do2.max())[i]) for i in range(2)]
    do_ph2 = np.sum(relu_np(do2), axis=0)  #!!
    id_sum_ph2 = int(np.where(do_ph2==do_ph2.max())[0])
    do_th2 = np.sum(relu_np(do1), axis=1)
    id_sum_th2 = int(np.where(do_th2==do_th2.max())[0])

    #--
    do_ph2 = (do_ph2 -  do_ph2.mean())/do_ph2.std()
    do_th2 = (do_th2 -  do_th2.mean())/do_th2.std()
    #--
